/**
 * Keycode data extracted from the VIA webapp source (src/utils/key.ts).
 * This is a static copy of all keycode categories and their keycodes.
 */
...
function buildLayerMenu(): IKeycodeMenu {
  ...
  const expandedKeycodes = layerTemplates.flatMap((keycode) => {
    const res: IKeycode[] = [];
    for (let idx = 0; idx < 10; idx++) {
      res.push({
        ...keycode,
        name: keycode.name + `(${idx})`,
        title: (keycode.title || '').replace('layer', `layer ${idx}`),
        code: keycode.code.replace('layer', `${idx}`),
      });
    }
    return res;
  });
  ...
  return {
    ...
    label: 'Layers',
    width: 'label',
    keycodes: [...hardCodedKeycodes, ...expandedKeycodes],
  };
}
...
/**
 * Build the master "All Keycodes" list: flattens every category into one list,
 * de-duplicated by code.
 */
export function getAllKeycodes(): IKeycode[] {
  const categories = getKeycodes();
  ...
  for (const cat of categories) {
    for (const kc of cat.keycodes) {
      if (!seen.has(kc.code)) {
        seen.add(kc.code);
        // Tag each keycode with its source category for display
        all.push({
          ...kc,
          title: kc.title
            ? `[${cat.label}] ${kc.title}`
